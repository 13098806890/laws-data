...
条文总数约 **78,000 条**，法条间引用关系 **6,452 条**（跨法引用 3,674 条，本法自引 2,778 条），解析率 98.8%。
...
7. **导出导航菜单**：生成 `law_menu.json`（全量，按法律部门分组）和 `flk_menu.json`（208 部，按 6 个法考科目排列），供 iOS app 侧边栏使用。
...
**当前统计**：6,452 条引用（跨法 3,674 条，自引 2,778 条），解析率 98.8%，78 条因目标法律未收录而无法解析。
...
- `法考/` 目录：按 6 个法考科目分子目录，共 208 个文件（`render_flk.py`）
